...
export default function EditProjectModal({ isOpen, onClose, organizationId, workspaceId, project }) {
  ...
  useEffect(() => {
    if (project && isOpen) {
      ...
      setHealth(project.health || 'healthy');
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-bg-primary/80 backdrop-blur-sm overflow-y-auto">
          ...
          >
            <div className="flex justify-between items-center mb-4">
              <div>
                <h2 className="text-lg font-bold text-text-primary">Edit Project Settings</h2>
                <p className="text-xs text-text-secondary">Update key metadata, status, and timeline configuration.</p>
              </div>
              <button
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                ...
                <div className="space-y-1">
                  <label className="text-[10px] uppercase font-bold text-text-tertiary">Status</label>
                  <select
                    value={status}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] uppercase font-bold text-text-tertiary">Health Status</label>
                  <select
                    value={health}
                    onChange={(e) => setHealth(e.target.value)}
                    className="w-full px-3 py-2 text-xs rounded-xl border border-border-primary bg-bg-primary text-text-primary focus:outline-none focus:border-brand-500 transition-colors"
                  >
                    <option value="healthy">Healthy ❤️</option>
              ...
              <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-border-primary">
                ...
                <button
                  type="submit"
                  disabled={updateMutation.isPending || !name.trim() || !key.trim() || !!dateError}
