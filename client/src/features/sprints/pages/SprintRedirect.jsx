...
/**
 * SprintRedirect — Smart router component for the /sprints route in sidebar.
 * Automatically forwards to the first active project's Sprint Hub,
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-text-tertiary">
        ...
        <span className="text-xs font-semibold">Opening Sprint Hub...</span>
      </div>
    );
  }

  // If projects exist, automatically redirect to the first project's sprint dashboard
  if (projects.length > 0) {
    return <Navigate to={`/projects/${projects[0]._id}/sprints`} replace />;
  }
  ...
  return (
    <div className="max-w-2xl mx-auto my-16 p-8 rounded-3xl border border-border-primary bg-bg-secondary/40 backdrop-blur-xl text-center space-y-6 shadow-xl">
      ...
      <div className="space-y-2">
        <h2 className="text-xl font-bold text-text-primary">No Active Projects</h2>
        <p className="text-xs text-text-secondary leading-relaxed max-w-md mx-auto">
          To plan iterations, track velocity, and manage sprints, create a project first.
        </p>
      </div>
